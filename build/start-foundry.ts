...
const nodeEntryPoint =
    fvttVersion < 13
        ? path.resolve(appPath, 'resources', 'app', 'main.js')
...
const startFoundry = async () => {
    try {
        if (fs.existsSync(windowsExecPath)) {
            ...
            await spawn(
                `node`,
                [nodeEntryPoint, `--dataPath=${dataPath}`, `--world=${world}`],
                {
                    stdio: 'inherit',
                },
            )
            ...
            console.error(
                `Cannot start FoundryVTT. "${appPath}" is not a valid Foundry path.`,
            )
            process.exit(1)
        }
    } catch (error) {
